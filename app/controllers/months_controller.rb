class MonthsController < ApplicationController
  ...
  def march
    ...
    @august = Expense.where('extract(month from created_at) = ?', '03')
    @total = @august.pluck(:amount).sum
    average_a
    pack
  end
  ...
  def april
    ...
    @august = Expense.where('extract(month from created_at) = ?', '04')
    @total = @august.pluck(:amount).sum
    average_b
    pack
  end
  ...
  def sept
    ...
    @august = current_user.expenses.where('extract(month from created_at) = ?', '09')
    @total = @august.pluck(:amount).sum
    average_b
    pack
  end
